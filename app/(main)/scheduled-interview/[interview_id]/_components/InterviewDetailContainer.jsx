    ...
    <div className="p-5">
      ...
      {Array.isArray(candidateList) && candidateList.length > 0 ? (
        candidateList.map((candidate, index) => {
          // Safely parse feedback JSON to extract communication rating
          let communicationRating = "N/A";
          try {
            const parsedFeedback = JSON.parse(candidate?.feedback || "{}");
            communicationRating =
              parsedFeedback?.feedback?.rating?.communication ?? "N/A";
          } catch (e) {
            // If parsing fails, leave rating as "N/A"
          }
