        ...
        <div className='p-5 bh-white rounded-lg border w-auto'>
            ...
            {!viewDetail ?
                <div className='flex gap-3 mt-5 content-center'>
                    <Button variant={'outline'} className={'w-31'} onClick={() => copyLink(url)}> <Copy /> Copy link</Button>
                    <Button onClick={onSend} className={'w-31'}> <Send /> Send </Button>
                </div>
                :
                <Link href={'/scheduled-interview/' + interview_id + '/detail'}>
                    <Button className={'mt-5 w-full'} variant='outline'> <ArrowRight /> View Detail </Button>
                </Link>
            }
